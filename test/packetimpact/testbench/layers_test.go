		...
		t.Run(tt.description, func(t *testing.T) {
			...
			for _, layer := range layers {
				...
				case *IPv6HopByHopOptionsExtHdr:
					layer.(*IPv6HopByHopOptionsExtHdr).NextHeader = nil
				case *IPv6DestinationOptionsExtHdr:
					layer.(*IPv6DestinationOptionsExtHdr).NextHeader = nil
				case *IPv6FragmentExtHdr:
					layer.(*IPv6FragmentExtHdr).NextHeader = nil
				case *ICMPv6:
					layer.(*ICMPv6).Checksum = nil
				}
			}
			gotBytes, err := layers.ToBytes()
			if err != nil {
